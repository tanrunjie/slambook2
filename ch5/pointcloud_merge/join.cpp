...
#include <boost/format.hpp> // formatting strings
...
typedef vector<Sophus::SE3d, Eigen::aligned_allocator<Sophus::SE3d>> TrajectoryType; // STL+eigen的内存分配写法
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        boost::format fmt("../%s/%d.%s"); // format字符串
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        for (int v = 0; v < color.rows; v++)
            for (int u = 0; u < color.cols; u++)
            {
                ...
                if (d == 0)
                    continue;
                Eigen::Vector3d point; // 像素坐标系到相机坐标系
                point[2] = double(d) / depthScale;
                point[0] = (u - cx) * point[2] / fx;
                point[1] = (v - cy) * point[2] / fy;
                Eigen::Vector3d pointWorld = T * point; // 相机坐标系到世界坐标系
    ...
    cout << " Total nums: " << pointcloud.size() << endl;
